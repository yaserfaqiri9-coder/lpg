...
### Full Suite
- کلِ Suite: **۱۰۴۱ Pass / ۴۲۱ Fail / ۱۴۶۲ Total**.
- **۴۰۶** از آن ۴۲۱ شکست، کلِ Collectionِ «Accounting PostgreSQL» است که در این محیط
  **قابل اجرا نیست**: خطای `28P01 password authentication failed for user "postgres"` — Fixture به
  اعتبارنامهٔ PostgreSQL نیاز دارد (`PTG_TEST_POSTGRES_ADMIN`) که در این محیط تنظیم نشده. این یک
  محدودیتِ محیط است، نه Regression.
- زیرمجموعهٔ **غیر-PG** (بدون آن ۱۳ کلاسِ Fixture): **۱۰۴۱ Pass / ۱۵ Fail / ۱۰۵۶ Total** در ۴۶ ثانیه.
- **هر ۱۵ شکستِ غیر-PG دقیقاً داخل همان Baselineِ ۱۶‌تایی هستند** (همگی UI/View/Loading؛ یکی از
  ۱۶ حتی حالا سبز شد). → **صفر شکست جدید از این کار.**
- **Baseline واقعیِ جدید (در این محیط، بدون PostgreSQL):** ۱۵ شکستِ قدیمیِ غیر-PG.
  برای تأییدِ کاملِ Baselineِ ۱۶ همراهِ Collectionِ PostgreSQL، Suite باید در محیطی با
  اعتبارنامهٔ درستِ `PTG_TEST_POSTGRES_ADMIN` اجرا شود.
